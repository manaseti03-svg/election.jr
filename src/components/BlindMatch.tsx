...
export default function BlindMatch() {
  const [cards, setCards] = useState<Policy[]>(INITIAL_POLICIES);
  const [scores, setScores] = useState<Record<Party, number>>({
    Progressive: 0,
    Conservative: 0,
    Libertarian: 0,
    Centrist: 0,
  ...
  const handleSwipe = (direction: 'left' | 'right') => {
    ...
    if (direction === 'right') {
      setScores(prev => ({
        ...prev,
        [activeCard.party]: prev[activeCard.party] + 1
      }));
    }
    ...
    <div className="w-full max-w-3xl mx-auto mt-24 mb-16 relative perspective-1000">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-lg text-slate-600 max-w-xl mx-auto font-medium">
          Swipe right if you agree, left if you disagree. We strip away the party labels to show you who you <span className="text-blue-600 font-bold">really</span> align with.
        </p>
      </div>
      ...
      <div className="relative w-full h-[400px] flex justify-center items-center">
        ...
        <AnimatePresence>
          {!isFinished && cards.map((policy, index) => {
          ...
          >
            ...
            <button 
              onClick={() => {
                ...
                setScores({ Progressive: 0, Conservative: 0, Libertarian: 0, Centrist: 0 });
              }}
